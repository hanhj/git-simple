...
\newline
\textbf{一阶常微分方程:}\\
可分离变量方程：$\frac{dy}{dx}=f(x)g(y)(g(y)\neq 0)$或$f(x)dx=g(y)dy$\\
齐次方程：$\frac{dy}{dx}=f(\frac{y}{x})$\\
齐次线性方程：$\frac{dy}{dx}+p(x)y=0$\\
\quad 解：$y=Ce^{- \int p(x)dx}$\\
非齐次线性方程:$\frac{dy}{dx}+p(x)y=q(x)$\\
\quad 解：公式$y=e^{- \int p(x)dx}\left(\int{q(x)e^{\int{p(x)dx}}+C}\right)$或常数变量法\\
伯努力方程：$\frac{dy}{dx}+p(x)y=q(x)y^n,n\neq0,1$\\
\quad 解：两边除以$y^n$,转换成非齐次线性方程，然后用公式法或常数变量法求解。
