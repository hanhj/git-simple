...
typedef struct _edma3_control{
    ...
    unsigned int tccNum    ;//Transmit Complete Code,this will useed by Chain Enable Register (CER[tcc]) 
    unsigned int edmaTC    ;
...
static edma3_control edma_ctrl[2]={
	[0]={//DMA
...
*				CC0 Complete Event 8,cpuInt 4
*				Error Int 56,cpuInt 5
**************************************************************************************
*/
...
void init_edma(void){
    ...
    Hwi_create(4,edma3_CC_0_ComplIsr,&hwi_para,&eb);//Use CPU Int 4
	hwi_para.eventId = SYS_INT_EDMA3_0_CC0_ERRINT;//EDMA3 CC0 ERR Interrutp Event 
    Hwi_create(5,edma3_CC_0_ErrIsr,&hwi_para,&eb);//Use CPU Int 4
}
void deinit_edma(){
...
*  @param[in]   ctrl_id edma3 control index,data is in edma_ctrl array;0 is
*  for edma_crtl[0],1 is for edma_ctrl[1]
*  @param[out]
*  @return  鎴愬姛锛岃繑鍥�锛屽惁鍒欒繑鍥�銆�*  @note
**************************************************************************************
*/
