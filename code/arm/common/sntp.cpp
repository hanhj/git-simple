...
CNtpTime::CNtpTime(CNtpTimePacket& packet)
{
  ...
  m_Time = ((unsigned int64) dwHigh) << 32;
  m_Time += dwLow;
}
...
double d;
  if (timespan >= 0)
	{
	  diff = ((unsigned int64) timespan) << 32;
	  frac= modf(timespan, &intpart);
	  diff += (unsigned int64) (frac * 0xFFFFFFFF);
      rVal.m_Time += diff;
	}
	else
	{
    	d = -timespan;
	 	diff = ((unsigned int64) d) << 32;
	  	frac = modf(d, &intpart);
		diff += (unsigned int64) (frac * 0xFFFFFFFF);
		rVal.m_Time -= diff;
	}
...
void CNtpTime::AddSpan(const double& timespan) 
{

  if (timespan >= 0)
	{
	  uint64 diff = ((unsigned int64) timespan) << 32;
    double intpart;
	...
	{
    double d = -timespan;
	  uint64 diff = ((unsigned int64) d) << 32;
    double intpart;
...
static void SetClientTime(CNtpTime NewTime)
{
	CNtpTime tmTime(NewTime+(double)(-timezone));
	SYSTEMTIME st = tmTime;
	timer.SetLocalTime(&st);
}

char deal_sntp( BUF_MUX *buf, uint16 port_id )
{
	...
	while(buf->recv.in_cnt!=buf->recv.out_cnt){
		data[count]=buf->recv.buf[buf->recv.out_cnt];
		count++;
		buf->recv.out_cnt++;
		if(count>=size){
			frame_ok=1;
	...
	if(frame_ok){
			...
			PFUNC(TEM_INFO ,DEBUG_TIMER,("Time was successfully retreived from NTP server\n"));
			...
			PFUNC(TEM_INFO ,DEBUG_TIMER,("                            DD/MM/YYYY  HH:MM:SS.MS\n"));
			PFUNC(TEM_INFO ,DEBUG_TIMER,("Client Originate Date was   %02d/%02d/%04d, %02d:%02d:%02d.%03d\n"), st1.wDay, st1.wMonth, st1.wYear, st1.wHour, st1.wMinute, st1.wSecond, st1.wMilliseconds);
			PFUNC(TEM_INFO ,DEBUG_TIMER,("Server Receive Date was     %02d/%02d/%04d, %02d:%02d:%02d.%03d\n"), st2.wDay, st2.wMonth, st2.wYear, st2.wHour, st2.wMinute, st2.wSecond, st2.wMilliseconds);
			PFUNC(TEM_INFO ,DEBUG_TIMER,("Server Transmit Date was    %02d/%02d/%04d, %02d:%02d:%02d.%03d\n"), st3.wDay, st3.wMonth, st3.wYear, st3.wHour, st3.wMinute, st3.wSecond, st3.wMilliseconds);
			PFUNC(TEM_INFO ,DEBUG_TIMER,("Client Destination Date was %02d/%02d/%04d, %02d:%02d:%02d.%03d\n"), st4.wDay, st4.wMonth, st4.wYear, st4.wHour, st4.wMinute, st4.wSecond, st4.wMilliseconds);
  			PFUNC(TEM_INFO ,DEBUG_TIMER,("Round trip delay was %f seconds\n"), response.m_RoundTripDelay);
  			PFUNC(TEM_INFO ,DEBUG_TIMER,("Local clock offset was %f seconds\n"), response.m_LocalClockOffset);
		...
		//	if (bSynchroniseClock)
			{
				if (fabs(response.m_LocalClockOffset) < 3600)
				{
					...
					PFUNC(TEM_INFO ,DEBUG_TIMER,("time difference was greater was 1 hour, not synchronizing clock\n"));
      		} 
      	frame_ok=0;	
      	count=0;
	}
	else{
		//printf(("Failed to retreive time from NTP server\n"));
		return 0;
	}
	return 1;	
}
