...
typedef struct time_
{
	...
	int   isdst;/**<Daylight   Saving   time   indicator*/
} SYSTIME;
...
typedef struct time_data {
    uint16 millisecond;//0-999毫秒
    uint8 minute;//IV--是否有效 0有效 1无效 占字节最高位 第七位备用 低6位为数据（0-59分--在GB_T 18657.4-2002 远动设备及系统 第五部分 传输规约 第四篇 应用信息元素的编码和定义中例图有误写成0-99）
    uint8 hour;//SU--是否夏令时 0标准时间，1夏令时 占字节最高位，第六、七位备用，低5位为数据（0-23小时）
    uint8 day;//day_week:week占字节高3位(1-7星期的天)，day占字节低5位 （1-31月的天）
    uint8 month;//高四位备用 month占低4位（1-12月）
    uint8 year;//最高位备用 year占低7位（0-99年）
}__attribute__ ((packed))  CP56Time2a;
