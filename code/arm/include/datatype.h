...
#include "user.h"
#include "os.h"
#define MAXYXDATA 1000//目前643
#define MAXYCDATA 500//目前294
#define MAXYKDATA 50//目前19
#define MAXPARADATA 500//目前326
...
#define PARA_5217  0x5217   ///*CP56Time2a格式终端时间+omapl138 timer counter*/        
...
class TerminalTime:public BaseDataType{
public:
	TerminalTime(){DataType = 34;}
	struct{
		CP56Time2a time;
	}__attribute__ ((packed)) Data;
};
//this datatype include omapl138 timer2 counter(64bit)
//for synchronize arm and dsp time
struct omapl138_timer_counter{
	uint32 counter12;
	uint32 counter34;
};
class TerminalTime2:public BaseDataType{
public:
	TerminalTime2(){DataType = 34;}
	struct{
		CP56Time2a time;
		struct omapl138_timer_counter counter;
	}__attribute__ ((packed)) Data;
};
...
typedef struct para_file_header {
    int16 Flag;
    int16 Num;
    int16 TotalLen;
    Item items[MAXPARADATA];
}PARA_FILE_HEADER;
...
TerminalTime d_5215;   ///*CP56Time2a格式终端时间*/        
PassWdPara d_5216;   //终端类型,三遥型：D30；二遥标准型：D21；二遥动作型：D22
TerminalTime2 d_5217;   ///*CP56Time2a格式终端时间+omapl138 timer counter*/        
