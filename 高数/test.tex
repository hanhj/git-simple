...
\section{附录1不定积分的常用方法} 
	...
	\subsection{常用形式}
		...
		2)分解\\
		参考例子:\\
			$\int \frac{dx}{ax^2+b}$,利用$\int\frac{dx}{x^2+a^2}=\frac{1}{|a|}arctan(\frac{x}{|a|})+C $和$\int\frac{dx}{x^2-a^2}=\frac{1}{2a}ln(|\frac{x-a}{x+a}|)+C $\\
			$\int \frac{xdx}{ax^2+b}$,凑微分\\
			$\int \frac{x^2dx}{ax^2+b}$，分解\\
			$\int \frac{dx}{x*(ax^2+b)}$，分解\\
			$\int \frac{dx}{x^2*(ax^2+b)}$，分解\\
			$\int \frac{dx}{x^3*(ax^2+b)}$，利用上一个分解的结果分解\\
		...
		\subsubsection{含$\sqrt{\pm\frac{x-a}{x-b}}$,或$\sqrt{(x-a)(b-x)}$}
		\subsubsection{含三角函数}
\end{flushleft}
\end{document}
